...
def callback(msg):
    ...
    if(i % 10) ==0:
         ...
         print("x0val: ", x_data[0],"   y0val: ", y_data[0], "   size of skel. Array: ", len(msg.points) )
         ...
         def polynomialFitting(x_vals,y_vals):
             n=10
             p = np.poly1d(np.polyfit(x_vals, y_vals, n))
             t = np.linspace(min(x_vals), max(x_vals), 100)
         ...
         polynomialFitting(x_data,y_data)
         plt.draw()
